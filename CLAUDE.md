...
- **Tab DCA gom về hai hàng card + ba popup, kích thước card KHÔNG đổi khi đổi coin (owner yêu cầu 2026-09-13):**
  ...
  - **Ví Coin trong 440px:** `table-layout:fixed` + `colgroup` 23/19/33/15/10% (đo: nội dung tự do phình 502px trong khung 402px). Bỏ hai nút ↑↓ (chỉ giữ xoá), tên coin ellipsis, dòng tỉ giá nhỏ bỏ hậu tố "USDT" vì dòng đậm ngay trên đã ghi đơn vị. `coinPriceCell()` giờ chỉ còn Ví Coin dùng.
