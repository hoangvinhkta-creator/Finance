...
- **Mỗi dòng trong 4 bảng dùng icon thay cho nút chữ** (owner yêu cầu 2026-09-13): sửa (bút chì), xoá (thùng rác), đã nhận/tất toán (dấu tick trong khung, class `.tag`), mở lại (mũi tên xoay ngược). Icon SVG để trong hằng `IC_EDIT/IC_DEL/IC_CHECK/IC_UNDO`, dựng qua `rowBtn(act, icon, title, cls)`. Mỗi hàng chỉ còn **một `<td class="rowcell">`** chứa `.rline` (tên — icon — số tiền, flexbox, tên bị `text-overflow:ellipsis` cắt gọn) để chiều cao hàng luôn cố định, đo đúng bằng nhau ở cả 4 card. `data-act` giữ nguyên (`edit/del/received/settle/reopen`) nên các handler click hiện có không đổi. Cột nhóm (`.c`, vốn đã ẩn trong `.ledger4`) và cột hành động (`.act`) đã xoá khỏi DOM luôn, không chỉ ẩn bằng CSS.
